...
class LogDataGenerator:
    ...
    def __init__(self, output_dir='data/raw', target_lines=5000):
        ...
        # 服务列表
        self.services = ['order-service', 'database-service', 'auth-service', 
    ...
    def _generate_anomaly_a(self, base_time, offset_seconds):
        ...
        # 在高并发时段插入 Connection Timeout 错误
    ...
    def generate_logs(self):
        ...
        while len(all_logs) < self.target_lines:
            ...
            if is_high_concurrency:
                # 高并发时段：异常概率更高
                anomaly_a_prob = 0.15  # 15% 概率出现异常A
                anomaly_b_prob = 0.05  # 5% 概率出现异常B
            else:
                # 正常时段：异常概率较低
                anomaly_a_prob = 0.03  # 3% 概率出现异常A
                anomaly_b_prob = 0.02  # 2% 概率出现异常B
